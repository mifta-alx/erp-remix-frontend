...
export const meta = () => {
  return [
    { title: "ERP-Edit Bills of Materials" },
    { name: "description", content: "Edit Bills of Materials" },
  ];
};
...
export const loader = async ({ params }) => {
  ...
  try {
    const [productsResponse, materialsResponse, bomsResponses] =
      await Promise.all([
        fetch(`${process.env.API_URL}/products`),
        fetch(`${process.env.API_URL}/materials`),
        fetch(`${process.env.API_URL}/boms/${params.bom_id}`),
      ]);
    if (!productsResponse.ok || !materialsResponse.ok || !bomsResponses.ok) {
      ...
      } else if (!bomsResponses.ok) {
        status = bomsResponses.status;
        if (status === 404) {
          errorMessage = "Bills of Materials Not Found";
          errorDescription = "The BoM you're looking for do not exist.";
        }
      }
    ...
    const [products, materials, boms] = await Promise.all([
...
export default function EditBom() {
  ...
  useEffect(() => {
    ...
    setSearchTerm(`[${boms.product.internal_reference}] ${boms.product.name}`);
  }, [boms]);
                  ...
                  <div>
                    ...
                    <div ref={dropdownRef} className="relative w-full">
                      ...
                      {isDropdownVisible && (
                        <div
                          id="dropdown"
                          className="z-10 absolute bg-white divide-y divide-gray-100 rounded-lg shadow-md w-full max-h-32 overflow-y-auto mt-2 dark:bg-gray-700"
                        >
                          <ul
