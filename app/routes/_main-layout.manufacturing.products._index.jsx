import { ChevronRight, House, Package, Plus, Search } from "lucide-react";
...
export const loader = async () => {
  ...
  try {
    ...
    return { error: false, products: products.data, node_env };
  } catch (error) {
    return {
      error: true,
      status: 500,
...
export default function Products() {
  const { error, products, message, description, status, node_env } =
    useLoaderData();
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start w-full">
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 w-full sm:w-fit justify-end">
              ...
              <div className="flex flex-row gap-4 w-full sm:w-fit">
                {products.length > 0 && node_env === "production" && (
                  <Link
